...
* ``@macro`` registers a *command* macro — a function ``def name(ctx): ...`` that
  ...
  (``qcell macro run``), the TUI (``:macro name``), or the GUI command palette.
...
with ``--macros``. Executing macros runs arbitrary Python: qcell is not a
...
log = logging.getLogger("qcell.macros")
